...
def tempdir(tmpdir_factory):
    return tmpdir_factory.mktemp("alr_deterministic")


@pytest.mark.parametrize("case", CASES, ids=[x[0] for x in CASES])
def test_alr_deterministic(case, tempdir):
    name, parameters = case
    test_file = tempdir.join(f"test-{name}.xlsx")
    expected_file = os.path.join(
        "tests", "policy_models", "alr_deterministic_audit_files", f"expected-{name}.xlsx"
    )
    ALRDeterministicPolicyModel(**parameters).audit(test_file)
    exlcude_list = [
