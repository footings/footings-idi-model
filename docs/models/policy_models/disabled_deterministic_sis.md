...
```{eval-rst}
.. autoclass:: footings_idi_model.models.DValSisRPMD
```
...
```{code-cell} ipython3
import pandas as pd
from footings_idi_model.models import DValSisRPMD
...
```{code-cell} ipython3
model.audit("Audit-DValSisRPMD.xlsx")
```

The audit file can be downloaded {download}`here.<./Audit-DValSisRPMD.xlsx>`
